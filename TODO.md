...
- [x] Normalize DISCONNECT reason handling
  ...
  - [x] Distinguish admin/server close reason codes.
...
- [x] Add a pluggable authenticator trait.
- [x] Support username/password authentication.
- [x] Add ACL hooks for publish and subscribe authorization.
...
- [x] Add authorization tests.
  ...
  - [x] Rejected SUBSCRIBE once ACL hooks exist.
  - [x] Rejected PUBLISH once ACL hooks exist.
...
- [x] Add inflight retransmission timers for QoS 1/2.
...
- [x] Add graceful shutdown policy for active MQTT sessions.
  - [x] Stop accepting work on Ctrl-C and wait for the TCP server task to exit.
  - [x] Define whether active sessions receive server DISCONNECT packets, how long to drain in-flight work, and which reason code/properties are emitted.
